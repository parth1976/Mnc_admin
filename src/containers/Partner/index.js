...
import DataDrawer from './question';
...
const Company = () => {
  ...
  const fetchData = () => {
    ...
    callAPI("POST", `${BASE_URL}/admin/firstGame/level/getLevels`, filter)
      .then((res) => {
  ...
  const handleCreateCompany = (values) => {
    const url = selctedId ? `/admin/partner/${selctedId}` : `/admin/firstGame/level/create`
  ...
  const handleUserDelete = () => {
    callAPI("DELETE", `${BASE_URL}/admin/firstGame/level/deleteLevels`, { ids: selectedRowKeys })
      .then((res) => {
        if (res) {
          setSelectedRowKeys([]);

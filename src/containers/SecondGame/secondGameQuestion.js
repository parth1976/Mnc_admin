...
const DataDrawer = ({ levelId, visible, onClose }) => {
    ...
    const fetchData = async (id) => {
        setLoading(true);
        try {
            callAPI('POST', `${BASE_URL}/admin/secondGame/questions/getQuestions`, { filter: { level: levelId } })
                .then((res) => {
    ...
    const handleCreateQuestion = (values) => {
        ...
        callAPI("POST", `${BASE_URL}${url}`, { level: levelId, ...values })
            .then((res) => {
                if (res) {
                    setIsOpen(false);
                    fetchData();
    ...
    const columns = [
        ...
        { title: "Question", dataIndex: "questionText", key: "question" },
        {
